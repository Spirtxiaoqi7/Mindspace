...
def test_platform_failures_are_isolated_and_indexed_evidence_is_honest() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if host == "search.brave.com":
            return httpx.Response(
                200,
                text=(
                    '<div data-type="web"><a class="l1" href="https://x.com/a/status/1">'
                    '<div class="search-snippet-title">indexed X post</div></a>'
                    '<div class="generic-snippet"><div class="content">indexed summary</div></div>'
                ),
            )
    ...
    web = WebOrchestrator(
        config_provider=lambda: {"capabilities": {"max_web_pages": 0, "max_web_results": 8}},
        http_transport=httpx.MockTransport(handler),
    )
    result = web.execute(
        WebQuery(query="release", platforms=["github", "bilibili", "bluesky", "mastodon", "reddit", "x"])
    )
...
def test_weather_uses_structured_provider_before_search_pages() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.host == "geocoding-api.open-meteo.com":
            return httpx.Response(
                ...
                json={
                    "results": [
                        {"name": "西安", "admin1": "陕西", "country": "中国", "latitude": 34.26, "longitude": 108.94}
                    ]
                },
            )
    ...
    web = WebOrchestrator(
        config_provider=lambda: {"capabilities": {"max_web_results": 8}}, http_transport=httpx.MockTransport(handler)
    )
...
def test_platform_coverage_requires_target_evidence_and_strict_queries_need_timestamps() -> None:
    ...
    xia_home = WebSource(
        url="https://www.xiaohongshu.com/", title="小红书", platform="xiaohongshu", freshness="search_index"
    )
    xia_note = WebSource(
    ...
    assert (
        finalize_result(WebQuery(query="小红书帖子", platforms=["xiaohongshu"]), [xia_home], []).coverage
        == "unavailable"
    )
    assert (
        finalize_result(WebQuery(query="小红书帖子", platforms=["xiaohongshu"]), [xia_note], []).coverage == "indexed"
    )

    official_without_date = WebSource(
    ...
    assert (
        finalize_result(
            WebQuery(query="官方最近一周", scope="official", recency="week"), [official_without_date], []
        ).coverage
        == "partial"
    )
...
def test_official_brand_filter_rejects_lookalikes_and_marks_only_canonical_domains_authoritative() -> None:
    canonical = WebSource(
        url="https://api.deepseek.com/news",
    ...
    result = finalize_result(
        WebQuery(query="DeepSeek 官方最近一周", scope="official", recency="week"), [canonical, *mirrors], []
    )
...
def test_brave_rate_limit_enters_cooldown_and_uses_bing_without_repeated_failure() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.host == "www.bing.com":
            ...
            return httpx.Response(
                200,
                text='<li class="b_algo"><h2><a href="https://example.com/item">fallback</a></h2><p>summary</p></li>',
            )
        raise AssertionError(request.url)

    SearchProvider._brave_cooldown_until = 0.0
    search = SearchProvider(
        WebOrchestrator(config_provider=lambda: {}, http_transport=httpx.MockTransport(handler)).http, lambda: {}
    )
    try:
...
def test_natural_language_github_query_resolves_langgraph_and_reads_latest_release() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/repos/langchain-ai/langgraph/releases/latest":
            return httpx.Response(
                200,
                json={
                    "html_url": "https://github.com/langchain-ai/langgraph/releases/tag/1.2.3",
                    "tag_name": "1.2.3",
                    "published_at": "2026-08-10T00:00:00Z",
                    "author": {"login": "langchain-ai"},
                },
            )
        raise AssertionError(request.url)
    ...
    result = web.execute(
        WebQuery(
            query="帮我在 GitHub 查找 LangGraph 官方仓库最近发布版本和更新时间。",
            platforms=["github"],
            scope="developer",
            recency="week",
        )
    ...
    assert result.coverage == "complete"
    assert [(item.platform, item.url) for item in result.sources] == [
        ("github", "https://github.com/langchain-ai/langgraph/releases/tag/1.2.3")
    ]
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.host == "www.bing.com" and request.url.params.get("format") is None:
            return httpx.Response(
                200,
                text=(
                    '<li class="b_algo"><h2><a href="https://github.com/langchain-ai/langgraph">'
                    "LangGraph</a></h2><p>Stateful graph framework</p></li>"
                ),
            )
        raise AssertionError(request.url)
...
def test_bing_click_wrapper_is_html_unescaped_and_decoded_before_policy() -> None:
    ...
    raw = (
        ...
        f'{encoded}">OpenAI news</a></h2><p>OpenAI update</p></li>'
    )
...
def test_filtered_zero_results_are_successful_but_total_provider_failure_is_not() -> None:
    empty = finalize_result(
        WebQuery(query="DeepSeek 官方最近一周", scope="official", recency="week"), [], [], executed=True
    )
    failed = finalize_result(
        WebQuery(query="DeepSeek 官方最近一周", scope="official", recency="week"),
        [],
        [{"provider": "bing_html", "error": "HTTP 503"}],
        executed=False,
    )
...
def test_real_input_fixtures_drop_irrelevant_web_results_before_coverage() -> None:
    x_result = finalize_result(
        ...
        [
            WebSource(
                url="https://support.microsoft.com/help", title="Microsoft support", text="Windows help", platform="web"
            ),
            WebSource(
                url="https://x.com/OpenAI",
                title="OpenAI",
