...
def test_routing_force_allow_and_roleplay_suppression() -> None:
    ...
    assert (
        decide_retrieval(
            ChatRequest(message="那最新的呢", session_id="routing"),
            [{"role": "user", "content": "联网搜索 OpenAI 最新发布"}],
        ).mode
        == "force"
    )
    assert (
        decide_retrieval(
            ChatRequest(message="帮我在 GitHub 查找 LangGraph 官方仓库最近发布版本和更新时间。", session_id="routing")
        ).mode
        == "force"
    )
...
def test_ranking_keeps_diverse_relevant_sources() -> None:
    ranked = rank_sources(
        WebQuery(query="Mindspace release"),
        [
            WebSource(
                source_type="index",
                url="https://example.com/release",
                title="Mindspace release",
                evidence_level="indexed",
            ),
            WebSource(
                source_type="index",
                url="https://example.com/tag/release",
                title="Mindspace release",
                evidence_level="indexed",
            ),
            WebSource(
