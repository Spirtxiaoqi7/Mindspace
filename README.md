...
> 当前 Core / Web 版本：**0.7.4**（Launcher 0.5.54）
...
- 新安装的大型运行时、模型、数据与下载缓存默认跟随安装盘，旧 C 盘数据保持可识别，
  并可通过事务化操作迁移到安装盘旁的 `MindspaceData`。
- Launcher 增加可选组件包管理：安装、断点续传、修复、安全卸载、共享依赖保护和人物
  音色隔离；修复 Fun-ASR Nano 旧版错误模型目录并自动认领已有文件。
- 启动器读取系统 RAM 与 NVIDIA 显存，在下载和启动两处阻止不满足配置的本地服务；
  文字聊天和云端 TTS 不受影响。
- 完整安装包内嵌应用私有 CPython 3.11 与 uv，基础 Python 部署不再依赖用户访问
  GitHub 或 Astral，也不会继承系统 Python、Conda 或 venv 状态。
- 修复旧版运行中覆盖安装可能长期等待卸载器的问题；升级使用有界关闭、保留数据卸载和
  明确错误码，不再留下无法操作的隐藏弹窗。

## 0.7.0–0.7.3 重点

- 新增按角色隔离的“共同篇章”：角色日记、共同片段、场景背景与活动状态均带
  `character_id`，叙事内容不能作为人物档案写回证据。
- 二次元典藏手账资源扩展为 171 项正式资源，覆盖图标、贴纸、边框、纹理、状态图、
  场景图和封面，并提供可重复构建、压缩、校验和资源包安全安装基础。
- 场景从任务状态机中独立，聊天内可直接切换背景；只向当前轮注入一条临时场景说明，
  不增加模型调用，也不进入长期记忆。
- 新会话首轮优先完成生成与持久化，RAG 在后台预热后从下一轮恢复，空工具节点不再
  阻塞普通聊天。
- 修复灵感抽卡自定义性格覆盖、流式回复滚动、ASR 下载状态展示和 GPT-SoVITS 残缺
  环境恢复等用户侧问题。

## 0.6.0 重点

- 增加模式大厅、灵感抽卡工坊和多角色卡册。
- 角色档案、头像、会话、运行状态、记忆与历史召回按 `character_id` 隔离。
- 抽卡最多调用一次模型，失败使用本地合法模板；支持角色预览、编辑、复制、归档、
  版本恢复和 `.mindspace-card` 安全导入导出。
- 旧角色及历史会话通过事务化迁移绑定到“原有角色”，失败完整回滚。
