...
# ruff: noqa: E501 -- authoritative Chinese prompt lines must remain byte-stable.
...
_CATEGORY_ALIASES = {
    ...
    "ai相关": "ai_related",
    "角色相关": "ai_related",
    "relationship": "relationship_related",
    "关系": "relationship_related",
...
_PROFILE_MEMORY_REQUEST = re.compile(
    r"(?:记住|记一下|写进(?:档案|记忆)|保存|更新).{0,32}"
    r"(?:偏好|喜欢|不喜欢|讨厌|习惯|称呼|口味|爱好)|"
    r"(?:我|你|她|他).{0,20}(?:喜欢|不喜欢|讨厌|偏好|习惯|希望|想要)",
    re.IGNORECASE,
)
_MATCH_STOP_GRAMS = {
    "用户",
...
def event_memory_lane(message: str) -> bool:
    """Event writes are exclusive, except explicit profile preference writes."""
    from mindspace_graph.web.routing import infer_platforms

    return should_consider_event(message) and not bool(
        _EXTERNAL_REQUEST.search(message or "")
        or _PROFILE_MEMORY_REQUEST.search(message or "")
        or infer_platforms(message or "")
    )
...
def parse_event_operation(raw: str) -> dict[str, Any]:
    ...
    group = _GROUP_ALIASES.get(
        str(value.get("group") or "subject").strip().lower(), str(value.get("group") or "subject").strip().lower()
    )
    category = _CATEGORY_ALIASES.get(
        str(value.get("category") or "relationship_related").strip().lower(),
        str(value.get("category") or "relationship_related").strip().lower(),
    )
...
def build_event_extraction_messages(
    ...
    payload = {
        ...
        "recent_dialogue": [
            ...
            if str(item.get("role") or "") in {"user", "assistant"} and _clean_text(item.get("content"), 1200)
        ],
        "retrieved_context": list(retrieved_context or [])[:8],
...
结合 recent_dialogue 最近三轮连续对话判断，不要把 current_user 当成孤立句子。用户的简短确认可以结合前文成为证据，但必须能从连续对话中明确还原事件。
retrieved_context 是本轮实际召回的旧对话或长期记忆，structured_context 是当前权威档案。它们只用于补全指代、理解关系、识别重复和提高压缩质量，不能单独作为新增、修改或删除事件的证据。
写入的事件必须能由 recent_dialogue 中的用户陈述或确认支持；如果只有召回或结构化信息支持，返回 none。
...
class EventMemoryStore:
    ...
    def _empty(character_id: str) -> dict[str, Any]:
        return {
            ...
            "revision": 0,
            "pending": [],
            "subjects": {category: None for category in SUBJECT_CATEGORIES},
            "history": [],
            "updated_at": _now(),
        }
    ...
    def prompt_snapshot(self, character_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "pending": list(value.get("pending", []))[:PENDING_LIMIT],
            "subjects": subjects,
            "active_count": len(value.get("pending", [])) + len(subjects),
        }
        ...
        *,
        ...
        with (
            ...
            self.database.transaction(
                operation="event_memory_write", details={"character_id": character_id, "action": action}
            ),
        ):
            value = self.snapshot(character_id)
            now = _now()
            ...
            if action == "add":
                event = {
                    ...
                    "title": _clean_text(operation["title"], 20),
                    "summary": _clean_text(operation["summary"], 160),
                    "status": "active",
                    "due_at": operation.get("due_at"),
                    "importance": max(1, min(3, int(operation.get("importance") or 2))),
                    "source_session_id": session_id,
                    "source_message_ids": list(source_message_ids or [])[:4],
                    "created_at": now,
                    "updated_at": now,
                }
                if event["group"] == "pending":
                    ...
                    if duplicate is not None:
                        ...
                        if len(value["pending"]) >= PENDING_LIMIT:
                            victim = min(
                                value["pending"],
                                key=lambda item: (int(item.get("importance") or 1), str(item.get("updated_at") or "")),
                            )
                    ...
                    event.update(
                        {
                            ...
                            "importance": max(1, min(3, int(operation.get("importance") or 2))),
                            "updated_at": now,
                            "source_session_id": session_id or event.get("source_session_id", ""),
...
class EventMemoryWritebackService:
    ...
    def kick(self, request: ChatRequest, response: ChatResponse) -> None:
        ...
        self._tasks[key] = asyncio.create_task(
            self._run(key, request, response), name=f"event-memory-{request.session_id[:10]}-{request.round}"
        )
    ...
    def _recent_dialogue(self, request: ChatRequest) -> list[dict[str, Any]]:
        messages = [
            item
            for item in self.dependencies.sessions.load_all(request.session_id)
            if str(item.get("role") or "") in {"user", "assistant"}
            and int(item.get("round") or 0) <= request.round
            and _clean_text(item.get("content") or item.get("message"), 1200)
        ]
        rounds: list[int] = []
        ...
        return [
            {
                "round": int(item.get("round") or 0),
                "role": str(item.get("role") or ""),
                "content": _clean_text(item.get("content") or item.get("message"), 1200),
                "message_id": str(item.get("message_id") or item.get("id") or ""),
            }
            for item in messages
            if int(item.get("round") or 0) in selected
        ][-8:]

    @staticmethod
    def _message_ids(recent_dialogue: list[dict[str, Any]], response: ChatResponse) -> list[str]:
        ids: list[str] = []
    ...
    async def _run(self, key: str, request: ChatRequest, response: ChatResponse) -> None:
        ...
        try:
            async with lock:
                ...
                recent_dialogue = self._recent_dialogue(request)
                profiles = self.dependencies.profiles.load_bundle(request.character_id)
                structured_context = profiles.model_dump(mode="json") if hasattr(profiles, "model_dump") else {}
                retrieved_context = list((response.writeback_context or {}).get("retrieval") or [])[:8]
                ...
                user_evidence = "\n".join(
                    str(item.get("content") or "") for item in recent_dialogue if item.get("role") == "user"
                )
                operation = resolve_event_target(operation, snapshot, user_evidence or request.message)
                result = self.store.apply(
                    request.character_id,
                    operation,
                    session_id=request.session_id,
                    source_message_ids=self._message_ids(recent_dialogue, response),
                )
                self.dependencies.audit.record(
                    "event_memory_completed" if result["changed"] else "event_memory_skipped",
                    {
                        ...
                        "context_rounds": sorted({int(item.get("round") or 0) for item in recent_dialogue}),
                        "retrieval_items": len(retrieved_context),
                        "structured_revisions": dict(structured_context.get("revisions") or {}),
                    },
                )
