...
def _is_target_source(source: WebSource, platform: str) -> bool:
    ...
    if platform == "github":
        return (
            ...
            and source.evidence_level.startswith("github_rest")
            and (host == "github.com" or host.endswith(".github.com"))
        )
    if platform == "x":
        return (
            source.platform == "x"
            and source.evidence_level in {"x_oembed", "x_syndication", "indexed_summary"}
            and bool(_X_STATUS.search(f"{host}{urlsplit(source.url).path}"))
        )
    if platform == "xiaohongshu":
        return (
            source.platform == "xiaohongshu"
            and source.evidence_level == "indexed_summary"
            and bool(_XIAOHONGSHU_NOTE.search(f"{host}{urlsplit(source.url).path}"))
        )
...
def _is_platform_secondary(source: WebSource, targets: set[str]) -> bool:
    ...
    if "x" in targets:
        return (
            source.platform == "x"
            and (host in {"x.com", "twitter.com"} or host.endswith(".x.com") or host.endswith(".twitter.com"))
            and bool(path)
        )
...
def finalize_result(
    query: WebQuery, sources: list[WebSource], failures: list[dict[str, str]], *, executed: bool = True
) -> WebResult:
    ...
    for item in sources:
        if (
            not item.url
            or not (item.title or item.text)
            or not source_matches_entities(item.url, item.title, item.text, intent)
            or is_high_risk_official_imposter(item.url, intent)
        ):
            continue
    ...
    secondary = [
        item for item in values if item not in primary and (not targets or _is_platform_secondary(item, targets))
    ]
    ...
    complete = (
        ...
        and any(item.freshness == "official_api" for item in selected)
    )
    ...
    missing_official_brand = (
        query.scope == "official"
        and bool(requested_brand_keys(intent))
        and not any(item.authority.startswith("official_") for item in selected)
    )
    coverage = (
        "complete"
        if complete
        else "unavailable"
        if (targets and not primary) or missing_official_brand
        else "indexed"
        if indexed and not missing_targets
        else "partial"
    )
    freshness = "official_api" if complete else "search_index" if indexed else "mixed"
    return WebResult(
        query=query.query, status="success", coverage=coverage, freshness=freshness, sources=selected, failures=failures
    )
