...
_PLATFORM_PATTERNS = {
    "x": r"推文|Twitter|推特|(?:^|[\s在从])X(?:\s*(?:上|里|平台))?(?=[\s，、。！？?!]|查|搜|找|看|打开|发布|发|的|\(|$)",
...
def _contextual_external_followup(
    text: str, history: list[dict[str, Any]] | None
...
def decide_retrieval(request: ChatRequest, history: list[dict[str, Any]] | None = None) -> RetrievalDecision:
    ...
    if contextual_followup is not None: return contextual_followup
    ...
    if _FRESH.search(text) and (_EXTERNAL.search(text) or platforms) and not re.search(r"(?:用|写).{0,20}(?:脚本|代码)", text): return RetrievalDecision(mode="force", scope=scope, reason_codes=["time_sensitive_external_fact"], confidence=.94, query=text, platforms=platforms, recency=recency)
