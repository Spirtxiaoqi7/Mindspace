...
FINAL_AFTER_TOOL_PROTOCOL = (
    ...
    "若工具成功且有来源，必须先使用返回数据直接回答用户的问题，"
    "不得说没有查过、没有留意、可以再帮用户查，也不得用角色闲聊回避结果。"
    "先理解结构化数据，再用自然、连贯且符合角色语气的语言回答；"
    "不得复制 JSON、字段名、覆盖状态或内部数据结构到回复正文。"
    "保持角色语气，但事实答案优先；只总结来源中明确给出的内容和时间。"
