...
第 `N+1` 轮复用同一 Epoch 的稳定 System 与 JSON，并仅选择最近 8 个 round 的
已确认原始对话；
...
原始会话无论是否达到压缩阈值，正文模型都只读取最近 8 轮。若这 8 轮本身仍超过
`context_window × hard_ratio - reserved_tokens`，当前主请求不会等待压缩模型，而是在
8 轮窗口内继续从后向前缩减。规范账本和页面原文不被截断。
...
前端先把第一条完整自然句送入 `/api/v1/audio/tts/stream`，保证低延迟开口。
实时语音会把全角/半角括号中的动作描写作为独立语音段；其余普通正文跨句、
跨段累计，到下一个括号或本轮结束时再提交。音频 PCM 通过播放 AudioWorklet
顺序消费，避免每段创建独立 `<audio>` 导致空隙。
...
`0.8s / 1.6s / 3.2s / 5s` 退避并保持 5 秒间隔持续恢复，直到连接成功或用户退出。
连接前先通过 `/audio/status` 确认 ASR Worker 已就绪，避免冷启动期间反复申请麦克风。
TTS 的响应、首包、流空闲、播放器启动和播放结束均有独立看门狗；冷启动可等待
最多 90 秒。Core 使用单通道锁保证本地 TTS 只有一个请求进入 Worker，前端只在
当前合成完成后准备一个后续段。失败会取消等待任务、清空待播队列并解除输入门。
语音页始终保留退出和手动恢复入口，系统不偷偷切换浏览器音色。

Launcher 对自己启动的 Core、ASR 和 TTS 子进程执行 `1s / 2.5s / 5s` 三次有界
崩溃恢复；连续失败后停止拉起并写入 `runtime-manager.jsonl`，避免无限重启争抢显存。
ASR CUDA 依赖校验运行在异步子进程中；Torch 冷导入期间 Electron 主线程继续处理
窗口重绘、刷新和停止操作，禁止用同步子进程把 Launcher 卡成“未响应”。
