...
## 18. 0.8.0 桌面交互收敛、接口容错与品牌统一

**问题与用户感受**：0.7.x 在功能累积后出现入口重复、最近会话挤压、重复新建会话、配置散落和
共同篇章占据核心界面的情况；OpenAI 兼容服务返回非标准正文或抽卡 JSON 不完整时，前端又会把
可恢复内容当作完全失败。Launcher 的完整环境扫描也阻塞了用户进入文字聊天。

**最终选择**：

- 主页优先恢复当前角色的最近有效会话，无可恢复会话时才新建；最近会话列表独立滚动并恢复删除入口；
- 左下角收敛为“人设”，复用人物设定三页并并列 API 配置；知识库进入设置，聊天区只保留场景；
- 完整移除共同篇章及其前端类型和测试入口，避免继续维护无价值的并行交互路径；
- OpenAI 兼容适配器接受更多正文载荷，抽卡按字段进行确定性提取和截断，不因单个字段失配丢弃全部结果；
- Core 默认采用快速启动，完整环境核验只在修复或显式验证时执行；
- Launcher、主页、抽卡和聊天统一视觉语言，所有单字母 `M` 品牌占位替换为聊天气泡与宝石三点图标；
- Live2D 在 0.8.0 仅保留 V1.0 预告入口，不提交或打包 SDK、模型、渲染器及下载能力。

**验证边界**：0.8.0 已完成 Core/Web 构建、TypeScript 检查、针对性 Python 测试、离线安装包内容
检查和最终 EXE 图标提取核对；未把本轮构建视为公开发布，也未执行代码签名和公网更新切换。
